"""SQLite persistence for runtime v2 execution history."""
...
_RAW_PAYLOAD_ENV_VAR = "ANKIOPS_LLM_V2_PERSIST_RAW_PAYLOADS"

_JOB_TABLE = "llm_job_v2"
_ITEM_TABLE = "llm_job_item_v2"
_ATTEMPT_TABLE = "llm_attempt_v2"
_ATTEMPT_PAYLOAD_TABLE = "llm_attempt_payload_v2"
...
class LlmDb:
    ...
    def _create_schema(self) -> None:
        ...
        with self._conn:
            ...
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_llm_job_v2_created "
                f"ON {_JOB_TABLE}(created_at DESC)"
            )
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_llm_job_v2_status "
                f"ON {_JOB_TABLE}(status)"
            )
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_llm_job_item_v2_job "
                f"ON {_ITEM_TABLE}(job_id)"
            )
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_llm_job_item_v2_note_key "
                f"ON {_ITEM_TABLE}(note_key)"
            )
            self._conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_llm_attempt_v2_item "
