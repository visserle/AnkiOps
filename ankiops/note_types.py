...
def _load_template_content(filename: str, templates_dir: Path | None = None) -> str | None:
    """Read a card template file. Returns None if not found."""
    if templates_dir:
...
def _load_template(filename: str, templates_dir: Path | None = None) -> str:
    """Read a card template file, raising FileNotFoundError if not found."""
    content = _load_template_content(filename, templates_dir)
    if content is None:
...
def _get_card_templates(config: NoteTypeConfig) -> list[dict[str, str]]:
    ...
    # Card 1 (Base)
    front = _load_template_content(f"{model_name}Front.template.anki", tdir)
    back = _load_template_content(f"{model_name}Back.template.anki", tdir)
    
    if not front or not back:
        raise FileNotFoundError(f"Missing base templates for {model_name}")

    first_card_name = "Cloze" if config.is_cloze else "Card 1"
    templates.append({"Name": first_card_name, "Front": front, "Back": back})

    # Card 2+ (Dynamic)
    i = 2
    while True:
        front_n = _load_template_content(f"{model_name}Front{i}.template.anki", tdir)
        back_n = _load_template_content(f"{model_name}Back{i}.template.anki", tdir)
        
        if front_n and back_n:
            templates.append({"Name": f"Card {i}", "Front": front_n, "Back": back_n})
            i += 1
        else:
            break
