...
def test_import_update_is_full_sync_across_states(case, world):
    key = f"imp-state-update-{case.state.lower()}"
    ...
    world.write_qa_deck("UpdateStateDeck", [("State Q", "New A", key)])
...
def test_import_delete_is_full_sync_across_states(case, world):
    key = f"imp-state-delete-{case.state.lower()}"
    note_id = world.add_qa_note(
...
def test_import_move_is_full_sync_across_states(case, world):
    key = f"imp-state-move-{case.state.lower()}"
    note_id = world.add_qa_note(
    ...
    world.write_qa_deck("TargetStateDeck", [("Move State Q", "Move State A", key)])
    ...
    if case.state == "RUN":
        ...
        # Without intact mappings, full sync still converges, but as delete+create.
        assert_summary(result.summary, created=1, updated=0, moved=0, deleted=1, errors=0)
        new_note_id = next(iter(world.mock_anki.notes.keys()))
        card_id = world.mock_anki.notes[new_note_id]["cards"][0]
        assert world.mock_anki.cards[card_id]["deckName"] == "TargetStateDeck"
...
def test_import_duplicate_key_conflict_fails_across_states(case, world):
    dup_key = f"dup-{case.state.lower()}"

    world.write_qa_deck("ConflictA", [("A", "A1", dup_key)])
    world.write_qa_deck("ConflictB", [("B", "B1", dup_key)])
...
def test_import_rename_subdeck_is_full_sync_across_states(case, world):
    key = f"imp-state-rename-{case.state.lower()}"
    note_id = world.add_qa_note(
    ...
    world.write_qa_deck("NewDeck::Sub", [("Rename Q", "Rename A", key)])
