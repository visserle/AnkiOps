...
def test_imp_fresh_create_004_assigns_keys_to_duplicate_first_line_notes(world):
    ...
    world.write_qa_deck(
        "DuplicateFirstLineDeck",
        [
            ("Same prompt", "First answer", None),
            ("Same prompt", "Second answer", None),
        ],
    )
    ...
    assert_summary(result.summary, created=2, updated=0, moved=0, deleted=0, errors=0)
    content = world.read_deck("DuplicateFirstLineDeck")
    assert content.count("<!-- note_key:") == 2
    assert content.count("<!-- note_type: AnkiOpsQA -->") == 2
    assert "A: First answer" in content
    assert "A: Second answer" in content
    assert len(world.mock_anki.notes) == 2
...
def test_imp_run_delete_002_deleted_deck_file_removes_only_managed_notes(world):
    ...
    with world.db_session() as db:
        ...
        assert "DeletedFileDeck" in world.mock_anki.decks
        assert db.resolve_deck_id("DeletedFileDeck") is None


def test_imp_run_delete_003_deleted_deck_file_removes_empty_anki_deck(world):
    note_key = "imp-run-delete-file-003"
    note_id = world.add_qa_note(
        deck_name="DeletedFileDeck",
        question="Managed Q",
        answer="Managed A",
    ...
    with world.db_session() as db:
        ...
        assert note_id not in world.mock_anki.notes
        assert "DeletedFileDeck" not in world.mock_anki.decks
        assert db.resolve_deck_id("DeletedFileDeck") is None
