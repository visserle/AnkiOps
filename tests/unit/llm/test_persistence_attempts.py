...
from ankiops.llm.domain.outcomes import (
    ProviderOutcome,
    ProviderOutcomeKind,
    ProviderUsage,
)
from ankiops.llm.domain.payloads import NoteUpdate
from ankiops.llm.persistence.attempts import AttemptRecorder
from ankiops.llm.persistence.db import LlmDb
from ankiops.llm.runtime.provider import PreparedProviderRequest
...
def test_record_success_persists_success_outcome_kind(tmp_path):
    ...
    try:
        ...
        row = db._conn.execute(
            "SELECT outcome_kind, contract_fingerprint FROM llm_attempt"
        ).fetchone()
...
def test_record_error_persists_refusal_outcome_kind(tmp_path):
    ...
    try:
        ...
        row = db._conn.execute(
            "SELECT outcome_kind, refusal_reason FROM llm_attempt"
        ).fetchone()
        assert row is not None
...
def test_record_error_maps_post_parse_note_error_to_validation_outcome_kind(tmp_path):
    ...
    try:
        ...
        row = db._conn.execute(
            "SELECT outcome_kind, parsed_update_json FROM llm_attempt"
        ).fetchone()
        assert row is not None
...
def test_record_error_persists_fatal_outcome_kind_from_context(tmp_path):
    ...
    try:
        ...
        row = db._conn.execute(
            "SELECT outcome_kind, error_message FROM llm_attempt"
        ).fetchone()
        assert row is not None
