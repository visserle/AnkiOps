...
def test_open_creates_schema_and_indexes(tmp_path):
    ...
    try:
        ...
        assert "idx_llm_attempt_item" in indexes
    finally:
        db.close()
...
def test_write_tx_rolls_back_attempt_and_payload(tmp_path):
    db = LlmDb.open(tmp_path, persist_raw_payloads=True)
    try:
        job_id = db.start_job(task_name="grammar", model="gpt", model_id="gpt-5.4")
        item_id = db.insert_job_item(
        ...
        attempts = db._conn.execute("SELECT COUNT(*) FROM llm_attempt").fetchone()[0]
